...
from .config import load_config, MODELS, REASONING_MODELS
from .api import DeepSeekClient, APIError, AuthenticationError, RateLimitError
...
class ChatApp:
    ...
    def _send_message(self, text: str):
        ...
        try:
            ...
            self.console.print(
                self.ui.render_error("Invalid API key. Check your DEEPSEEK_API_KEY in .env")
            )
            self._rollback_user_message()
    ...
    def cmd_temp(self, args: str):
        ...
        try:
            ...
            if not 0.0 <= val <= 2.0:
                ...
                return
            if self.current_model in REASONING_MODELS:
                self.console.print("  [yellow]Temperature not supported for reasoning models[/yellow]\n")
                return
    ...
    def cmd_top_p(self, args: str):
        ...
        try:
            ...
            if not 0.0 <= val <= 1.0:
                ...
                return
            if self.current_model in REASONING_MODELS:
                self.console.print("  [yellow]Top-p not supported for reasoning models[/yellow]\n")
                return
    ...
    def cmd_maxtokens(self, args: str):
        ...
        try:
            ...
            if val < 1 or val > 65536:
                self.console.print("  [yellow]Max tokens must be between 1 and 65536[/yellow]\n")
                return
    ...
    def cmd_info(self, args: str):
        is_reasoner = self.current_model in REASONING_MODELS
        info = (
            f"  Model:      [bold cyan]{self.current_model}[/bold cyan]\n"
            f"  Session:    [bold]#{self.session_id}[/bold]\n"
        ...
        if not is_reasoner:
            ...
            info += "  Temperature: [dim]N/A (reasoner)[/dim]\n  Top-p:      [dim]N/A (reasoner)[/dim]"
