...
from .api import DeepSeekClient, APIError
...
class StreamController:
    ...
    def __init__(self, client: DeepSeekClient, ui: UIRenderer, console: Console, config: AppConfig):
        self.client = client
        self.ui = ui
            ...
            with Live(
                ...
                for chunk in self.client.stream_chat(messages, model, **overrides):
                    if isinstance(chunk, dict) and "reasoning" in chunk:
                        ...
                        live.update(self.ui.render_streaming(
                            accumulated, model, reasoning=accumulated_reasoning
                        ))
                ...
                if api_response:
                    live.update(self.ui.render_response(api_response, model))
        except APIError:
